...
tr2g = {}
trlist = []
with open("/data/MSc/2020/MA5112/scRNA-Seq/assets/tx2gene.txt") as f:
    for line in f:
        l = line.split()
        tr2g[l[0]] = l[1]
        trlist.append(l[0])
...
ecs = {}
with open('/data/MSc/2020/MA5112/scRNA-Seq/analysis/sorted/bus_output/matrix.ec') as f:
    for line in f:
        l = line.split()
        ec = int(l[0])
...
pbar=None
pumi=None
with open('/data/MSc/2020/MA5112/scRNA-Seq/analysis/counts/output.sort.txt') as f:
    gs = set()
    for line in f:
        l = line.split()
...
#fig.savefig('seurat/gene_hist.png')

outfile = 'seurat/matrix.mtx'
...
with open('seurat/genes.tsv','w') as of:
    for g,gn in gl:
        of.write("%s\t%s\n"%(g,gn))

with open('seurat/barcodes.tsv','w') as of:
