...
We will make plots for a handful of genes returned as significant in the lung vs control contrast. Select the top 3 up and down regulated de genes:
```{R, message=F, warning=F}
up <- res1[order(-res1$log2FoldChange),]
up_list <- head(rownames(up), n=3L)

down <- res1[order(res1$log2FoldChange),]
down_list <- head(rownames(down), n=3L)

gene_list <- c(up_list, down_list)
gene_list
```

## T-test
Now create a for loop (extracting counts in long format using `plotCounts`):
```{R,message=F, warning=F}
...
for(i in gene_list){
  ...
  p <- ggboxplot(d, x = "condition", y = "count",
                 fill = "condition", color = "black", palette = "jco",
                 ylab ="Normalized Counts", title = paste0(i),
                 xlab="", add = c("dotplot"),
...
## Anova
Create plots for control, lung and melanoma:
```{R, message=F, warning=F}
for(i in gene_list){
  ...
  # we need to bump the anova value above the other signif icons or else it is overlayed.
  # just add 10% of value ..
  max <- max(d$count)
  max <- max + (max*0.10)
  ...
  compare_means(count ~ condition,  data = d,
   ...
   p <- ggboxplot(d, x = "condition", y = "count",
                 ...
                 stat_compare_means(method = "anova", label.y = max) +        
